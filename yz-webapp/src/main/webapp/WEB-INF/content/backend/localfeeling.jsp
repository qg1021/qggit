...
<head>
	...
	function onBatchDelete(){
		...
		if(confirm("确认删除？")){
			$("#mainForm").attr("action","localfeeling!batchDelete.action?mtype=${mtype}").submit();
		}
	}
	function onPublish(){
		var  isChecked=false;
		...
		if(confirm("确认发布？")){
			$("#mainForm").attr("action","localfeeling!publish.action?mtype=${mtype}").submit();
		}
	}
	function onCancel(){
		var  isChecked=false;
		...
		if(confirm("确认取消发布？")){
			$("#mainForm").attr("action","localfeeling!cancelPublish.action?mtype=${mtype}").submit();
		}
	}
	</script>
</head>
	...
	<div id="content">  
		...
		<div class="right_content">
			...
			<div class="pagehead01"></div>
    			...
    			<p class="ll mt15">
    				<button id="btnCreate" type="button" onclick="window.location.href='localfeeling!input.action?mtype=${mtype}'">新增</button>
	    			...
	    			<table class="tablebox">
	    				...
	    				<s:iterator value="page.result" status="st">
						   <tr>
						        ...
						        <td>${statusName}</td>
						        <td>
						        	<a href="localfeeling!input.action?id=${id}&mtype=${mtype}">修改</a>&nbsp;&nbsp;
						        	<a href="javaScript:delRecord('localfeeling!delete.action?id=${id}&mtype=${mtype}');">删除</a>
						        </td>
						    </tr>
					    </s:iterator>
