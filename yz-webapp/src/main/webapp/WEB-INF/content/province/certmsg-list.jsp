...
<div id="message" style="line-height: 35px;">
	...
	<form action="${ctx}/province/certmsg!list.action" method="post" name="mainForm" id="mainForm">
	...
	<ul>
		<li><label>证书状态</label><s:select list="#{'false':'有效证书','true':'作废证书'}" cssStyle="width:150px;" name="filter_EQB_isUnused" id="filter_EQB_isUnused" value="#parameters.filter_EQB_isUnused" theme="simple" headerKey="" headerValue="－所有证书－"/> </li>
    ...
    <td>${examArea.name}</td>
    <td>${examSite.name}</td>
    <td><s:if test="unused"><span class="colorred">已作废</span></s:if><s:else>有效</s:else></td>
    <td><s:if test="unused">作废证书</s:if><s:else><a href="${ctx}/province/certmsg!updateUsed.action?id=${id}&certCodeId=${certCodeId}">作废证书</a></s:else></td>
    </tr>   
    </s:iterator>  
    </table>  
    <!--照片弹出层 start-->
